...
export default function HabitGrid({
  ...
  const { dayHeaders, weeks } = useMemo(() => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      headers.push({ day: d, letter: WEEKDAY_LETTERS[dow], isToday: d === todayDate, dow });
      ...
      if (dow === 6 || d === daysInMonth) {
        currentWeek.end = d;
        wks.push({ ...currentWeek, span: currentWeek.end - currentWeek.start + 1 });
        weekNum++;
      ...
      <table className="habit-table">
        <thead>
          ...
          <tr className="week-header-row">
            <th className="col-sn" rowSpan={2}>S/N</th>
            <th className="col-habits" rowSpan={2}>Habits</th>
            {weeks.map((w, i) => (
              <th key={i} className="week-span" colSpan={w.span}>{w.label}</th>
            ))}
            <th className="col-done" rowSpan={2}>Done</th>
            <th className="col-total" rowSpan={2}>Total</th>
          </tr>
          {/* Day letters + numbers */}
          <tr className="day-header-row">
            {dayHeaders.map((h) => (
              <th key={h.day} className={`col-day ${h.isToday ? "today-col" : ""}`}>
              ...
              <tr key={habit.id}>
                ...
                <td className="cell-habit">
                  <div className="habit-name-row">
                    ...
                    <button className="habit-delete" onClick={() => removeHabit(habit.id)} title="Remove">✕</button>
                  </div>
                </td>
                {dayHeaders.map((h) => {
                ...
                <td className="cell-done">
                  <span className="done-text">{count} / {daysInMonth}</span>
                </td>
                <td className="cell-total">
                  <span className={`total-pct ${pct === 0 ? "zero" : "nonzero"}`}>{pct}%</span>
                </td>
              </tr>
            );
          })}
